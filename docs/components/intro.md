...
Bootstrap Themes are built with [Gulp](http://gulpjs.com), a build tool that compiles our CSS, JS, and docs with ease. You'll need to install Gulp before using our included `gulpfile.js`:
...
Once that is completed, launch the theme's docs and examples by running:

{% highlight bash %}
$ gulp docs
{% endhighlight %}

Also, you can modify your source files and run `gulp` to generate new local `dist/` files automatically. **Be aware that this replaces existing `dist/` files**, so proceed with caution.
...
- `toolkit.less` is the entry point for Less files - to build your own custom build, simply modify your local custom files or edit the includes listed here.
...
The basic template is a guideline for how to structure your pages when building with a Bootstrap Themes. Included below are all the necessary bits for using the theme's CSS and JS, as well as some friendly reminders.
...
<html lang="en">
  <head>
    ...
    <!-- Include the CSS -->
    <link href="dist/css/toolkit.min.css" rel="stylesheet">
